...
# 绘制收益曲线
def plot_performance(results_df, initial_capital=1000000):
	if len(results_df) == 0:
		print("没有回测结果可供绘图")
		return

	# 创建日期时间列用于绘图
	results_df['datetime'] = pd.to_datetime(results_df['date'].astype(str) + ' ' + results_df['time'].astype(str))

	# 设置图形
	fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 10))

	# 绘制资产曲线
	ax1.plot(results_df['datetime'], results_df['portfolio_value'], linewidth=1.5, color='blue')
	ax1.axhline(y=initial_capital, color='red', linestyle='--', alpha=0.7, label='初始资金')
	ax1.set_title('ETF轮动策略收益曲线', fontsize=16)
	...
	ax2.plot(results_df['datetime'], results_df['cumulative_returns'] * 100, linewidth=1.5, color='green')
	...
	plt.tight_layout()
	plt.savefig('etf_rotation_performance.png', dpi=300, bbox_inches='tight')
	plt.show()

	# 绘制每日收益分布直方图
	daily_returns = results_df.groupby('date')['portfolio_value'].last().pct_change().dropna()

	fig, ax = plt.subplots(figsize=(10, 6))
	ax.hist(daily_returns * 100, bins=50, alpha=0.7, color='steelblue', edgecolor='black')
	ax.axvline(x=daily_returns.mean() * 100, color='red', linestyle='--', linewidth=2,
			   label=f'均值: {daily_returns.mean() * 100:.2f}%')
	ax.set_title('每日收益分布', fontsize=16)
...
if len(results_df) > 0 and results_df.iloc[-1]['portfolio_value'] > 0:
	...
	print(f"收益曲线图已保存到 etf_rotation_performance.png 和 daily_returns_distribution.png")
